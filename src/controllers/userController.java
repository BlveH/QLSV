...
public class userController {
    ...
    public void updatePass(String name, String pass) {
        try {
            ...
            ConnectDB.stmt.executeUpdate(sq0);
    ...
    public boolean checkUser(String userName) {
        ...
        try {
            ...
            ResultSet rs = ConnectDB.stmt.executeQuery(sq0);
            i = rs.next();
        } catch (SQLException ex) {
        }
        return i;
    ...
    public void updateUser(User user) {
        String sq = "UPDATE `user` SET `userName`='" + user.getUserName() + "',`passWord`='" + user.getPassWord()
                + "' WHERE `userID`= " + user.getUserID() + ";";
        try {
            ConnectDB.stmt.executeUpdate(sq);
        } catch (Exception e) {
        }
    }
    ...
    public void insertUser(String name, String pass) {
        try {
            ...
            ConnectDB.stmt.executeUpdate(sq1);
        } catch (SQLException ex) {
            Logger.getLogger(userController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
